  ...
  return (
    <div className="flex flex-col space-x-3 border-y border-gray-100 p-5">
        ...
        <div className='mt-5 flex justify-between'>
            ...
            <div onClick={()=>{session && setCommentBoxVisible(!commentBoxVisible)}} className='flex cursor-pointer items-center space-x-3 text-gray-400 active:scale-75 transition-all  '>
                <ChatAlt2Icon  className='h-5 w-5'/>
                <p>{comments.length}</p>
            </div>

            <div className='flex cursor-pointer items-center space-x-3 text-gray-400 active:scale-75 transition-all '>
    ...
    {commentBoxVisible && (
        <form className="mt-3 flex space-x-3" onSubmit={handleSubmit}>
          ...
          <button
            disabled={!input}
            className="text-twitter__blue disabled:text-gray-200"
            type="submit"
          >
            Post
          </button>
        ...
        <div className="my-2 mt-5 max-h-44 space-y-5 overflow-y-scroll scrollbar-hide border-t border-gray-100 p-5">
          {comments.map((comment) => (
            <div key={comment._id} className="relative flex space-x-2">
              <hr className="absolute left-5 top-10 h-8 border-x border-twitter__blue/30" />
              <img
